...
*            N           均值滤波次数(范围:0~255)
*/
uint16 ad_ave(ADCn_e adcn, ADC_nbit bit, uint8 N) //均值滤波
{
    uint32 tmp = 0;
    uint8  i;
  //  ASSERT( ((adcn == ADC0) && (ch >= ADC0_SE8 && ch <= ADC0_SE18)) || ((adcn == ADC1) && (ch >= ADC1_SE4a && ch <= ADC1_SE17)) ) ; //使用断言检测ADCn_CHn是否正常

    for(i = 0; i < N; i++)
     tmp += adc_once(adcn ,  bit);
     tmp = tmp / N;
     return (uint16)tmp;
}
