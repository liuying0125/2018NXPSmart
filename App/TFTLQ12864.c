...
void TFT_LCD_Img_Binary(Site_t site, Size_t size, uint8 *img)
{
    ...
    int i ,j,z;
    //LCD_SetPos(0,0,80,60);	//定位显示像素位置
    /*LCD_SetPos(0,1,79,1);
    for(i=0;i<80;i++)
    {
        LCD_write_para16( BINARY_BGCOLOR );  
    }
    LCD_SetPos(0,2,79,2);
    for(i=0;i<80;i++)
    {
        LCD_write_para16( GREEN );  
    }
    LCD_SetPos(0,40,79,40);
    for(i=0;i<80;i++)
    {
        LCD_write_para16( BINARY_COLOR ); 
    }*/ 
    for(i=0;i<60;i++)
    {
        LCD_SetPos(0,i,79,i);
    ...
    for(i=0;i<80;i++)
    {
        for(j=0;j<4;j++)
        {
            if((pimg[i/8+j*10+560]&(0x80>>(i%8)))==0)
           {
              LCD_write_para16( BINARY_COLOR ); 
           }
           else
    ...
    /*while(total--)
    {
        ...
        while(bitindex--)
        {
            ...
            {
                LCD_write_para16( BINARY_COLOR );      //写图像数据
            }
            else
            {
                LCD_write_para16(  BINARY_BGCOLOR  );      //写图像数据
            }
        }
    } */
}

void TFT_LCD_Img_Binary_Zoom(Site_t site, Size_t size, uint8 *img, Size_t imgsize)
{
    ...
    LCD_SetPos(site.x,site.y,site.x+size.W,site.y+size.H);	//定位显示像素位置
